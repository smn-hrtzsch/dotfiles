    ...
    updateWebView() {
        let env = {
            ...
            settings: JSON.stringify({
                ...
                swapMouseButtons: config_1.config.previewSwapMouseButtons,
            }),
        };
        try {
            switch (this.status) {
                ...
                case previewStatus.processing:
                    ...
                    env.images = ["svg", "png"].reduce((p, c) => {
                        if (p)
                            return p;
                        let exported = (0, tools_2.calculateExportPath)(this.rendered, c);
                        exported = (0, tools_2.addFileIndex)(exported, 0, this.rendered.pageCount);
                        return fs.existsSync(exported) ? env.images = `<img src="${(0, tools_2.fileToBase64)(exported)}">` : "";
                    }, "");
                    this._uiPreview.show("preview.html", env);
                    break;
                default:
