...
function exportDiagram(diagram, format, savePath, bar) {
    if (bar) {
        bar.show();
        bar.text = (0, common_1.localize)(7, null, diagram.name + "." + format.split(":")[0]);
    }
    let renderTask = (0, appliedRender_1.appliedRender)(diagram.parentUri).render(diagram, format, savePath);
    if (!savePath) {
        // when exporting to buffer include map to make links clickable
        let mapTask = (0, appliedRender_1.appliedRender)(diagram.parentUri).getMapData(diagram, savePath);
    ...
    if (!config_1.config.exportMapFile(diagram.parentUri))
        return renderTask;
    ...
    let mapTask = (0, appliedRender_1.appliedRender)(diagram.parentUri).getMapData(diagram, cmapx);
    return combine(renderTask, mapTask);
}
